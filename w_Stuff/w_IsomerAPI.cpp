...
{
  ui->setupUi(this);
  ui->menuFile->addAction(tr("Edit database records..."), this, &IsomerAPI::editDatabaseRecords);
  auto recordsToolbar = addToolBar(tr("Database records"));
  recordsToolbar->setObjectName("databaseRecordsToolbar");
  recordsToolbar->setMovable(false);
  auto recordsButton = new QPushButton(tr("Edit Database Records..."), recordsToolbar);
  recordsButton->setMinimumHeight(34);
  recordsToolbar->addWidget(recordsButton);
  connect(recordsButton, &QPushButton::clicked, this, &IsomerAPI::editDatabaseRecords);
