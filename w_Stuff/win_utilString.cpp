...
QString stripHtmlColors(QString html);
...
{
#ifdef Q_OS_WIN
    static const QRegularExpression badChars(R"([<>:"/\\|?*\x00-\x1F])");
    name.replace(badChars, "_");
    // Trim trailing dots/spaces
    while (!name.isEmpty() && (name.endsWith(' ') || name.endsWith('.')))
        name.chop(1);
    // Avoid reserved names
    static const QRegularExpression reserved(
        R"(^(CON|PRN|AUX|NUL|COM[1-9]|LPT[1-9])(\..*)?$)", QRegularExpression::CaseInsensitiveOption);
    if (reserved.match(name).hasMatch())
        name.prepend('_');
#else
    static const QRegularExpression badChars(R"([/\x00])");
    name.replace(badChars, "_");
    if (name == "." || name == "..")
        name.prepend('_');
#endif
...
//WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW
// Remove color-related attributes and CSS properties, keep other markup intact.
QString stripHtmlColors(QString html)
{
  using RE = QRegularExpression;
    const auto I = RE::CaseInsensitiveOption;
    const auto D = RE::DotMatchesEverythingOption;

    // Remove HTML attributes color="..." or bgcolor="..."
    html.replace(RE(R"((\s)(color|bgcolor)\s*=\s*(".*?"|'.*?'|[^\s>]+))", I), QString());

    // Remove inline CSS color / background-color / border-color
    const char* cssProps[] = {
        R"((^|[;{\s])\s*color\s*:\s*[^;}"']+;?)",
        R"((^|[;{\s])\s*background-color\s*:\s*[^;}"']+;?)",
        R"((^|[;{\s])\s*border-color\s*:\s*[^;}"']+;?)"
    };
    for (auto p : cssProps)
        html.replace(RE(p, I | D), QString("\\1"));

    // Remove any now-empty style="" attributes
    html.replace(RE(R"(style\s*=\s*(['"])\s*\1)", I), QString());

    // Remove <font ...> and </font> tags entirely (keep their content)
    html.replace(RE(R"(<\s*font[^>]*>)", I), QString());
    html.replace(RE(R"(<\s*/\s*font\s*>)", I), QString());
